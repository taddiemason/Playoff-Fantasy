...
app.get('/api/standings', async (req, res) => {
  ...
  try {
    ...
    const [skaterData, goalieData] = await Promise.all([
      cachedFetch('skaters', `https://api.nhle.com/stats/rest/en/skater/summary?cayenneExp=${skaterExp}&sort=points&start=0&limit=500`),
      cachedFetch('goalies', `https://api.nhle.com/stats/rest/en/goalie/summary?cayenneExp=${goalieExp}&sort=wins&start=0&limit=200`)
    ]);
